...
function buildZoningProfile(zones: ZoningDistrict[]): ZoningProfile {
  ...
  // Special use districts: zones with commercial hours restrictions, or whose
  // general plan category explicitly indicates a special designation.
  ...
  // Height data is not present in the Zoning Districts dataset (ibu8-4ccn).
  // It is tracked in a separate Height and Bulk Districts dataset.
...
// ── Fallback mock data (used when live datasets return empty schemas) ──────────
//
// Both k55i-dnjd (Development Pipeline) and ibu8-4ccn (Zoning Districts) are
// federated from data.staff.sf.gov and return {} records via the public API.
// These fallbacks keep the AI briefing layer functional until dataset access
// is resolved.
